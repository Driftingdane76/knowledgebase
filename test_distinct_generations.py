...
def test_distinct_and_unique_generations(total_images=50):
    print("=" * 75)
    print(f"VERIFYING UNIQUENESS & DIVERSITY OF ALL {total_images} GENERATED TEST IMAGES")
    print("=" * 75)
    
    test_output_dir = os.path.join(BASE_DIR, "test_htmls", f"uniqueness_verification_{total_images}")
    ...
    with sync_playwright() as p:
        ...
        for i in range(1, total_images + 1):
            ...
            if cpr_matches:
                ...
                print(f"  ✓ Image {i:02d}/{total_images}: 100% Unique & Redacted | {len(ocr_data)} OCR tokens processed")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Verify uniqueness of generated mock images.")
    parser.add_argument("--count", type=int, default=50, help="Number of test images to generate (default: 50)")
    args = parser.parse_args()
    
    success = test_distinct_and_unique_generations(total_images=args.count)
    if not success:
        sys.exit(1)
