...
class TestMockPipelineStandard(unittest.TestCase):
    """
    Benchmark Suite 1: Standard Settings (num_beams=3).
    Renders 10 dynamic UI component screenshots via Playwright and evaluates
    Florence-2 baseline accuracy, CPR/Bank redaction recall, and execution latency.
    """

    @classmethod
    def setUpClass(cls):
        print("\n" + "=" * 80)
        print("STARTING TEST 1: FLORENCE-2 STANDARD PIPELINE (num_beams=3)")
        print("=" * 80)
        ...
        with sync_playwright() as p:
            ...
            for i in range(1, cls.num_cases + 1):
                ...
                # Run Florence-2 with num_beams=3
    ...
    def test_01_tokens_extracted_all_images(self):
        """Verify tokens were successfully read on all screenshots."""
        for res in self.results:
            self.assertGreater(res['tokens_count'], 0, f"Screenshot #{res['index']} failed to extract any OCR text.")

    def test_02_pii_redaction_presence(self):
        """Verify CPR / Bank masking executed across the dataset."""
        for res in self.results:
            # If the raw text contained CPR/Bank indicators, ensure sanitized text holds redacted tokens
            if "cpr" in res['raw_text'].lower() or "reg" in res['raw_text'].lower() or "konto" in res['raw_text'].lower():
                self.assertTrue(
                    ...
                    f"Screenshot #{res['index']} contained PII keywords but no redaction took place."
                )

    def test_03_print_summary(self):
        """Print baseline benchmark metrics."""
        avg_time = sum(r['elapsed'] for r in self.results) / len(self.results)
        print("\n" + "-" * 80)
        print(f"BASELINE SUMMARY (num_beams=3): Average Latency = {avg_time:.2f}s per screenshot")
        print("-" * 80 + "\n")
